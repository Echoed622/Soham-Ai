...
# STEP 3: Echo'S PERSONALITY 
#To Do 1 
BASE_SYSTEM_PROMPT = """
You are Echo, a friendly AI coding buddy for young learners! 🤖
- Fun, kind, and always encouraging
- Keep answers long and explanatory (8-20 sentences)
            ...
            messages    = [
                ...
                *st.session_state["messages"],          # 🧠 Full memory!
                {"role": "user", "content": (
                    ...
                    f'Answer using the search results. Keep Echo personality!'
                )}
            ]
        )
...
with st.sidebar:
    ...
    st.caption("Echo will read it and answer questions about it!")
    ...
    show_thinking  = st.toggle("Show Echo's thinking", value=True)
    st.markdown("---")
...
if user_input:
    ...
    with st.chat_message("assistant"):
        with st.spinner("🧠 Echo is thinking..."):
            try:
                ...
                if show_thinking:
                    if did_search:
                        with st.expander("🌐 Echo searched the web — see how!"):
                            #To Do 5
    ...
    # 6️⃣ Save Echo's reply to memory too!
